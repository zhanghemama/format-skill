...
**优先级红线：显式标题层级 > label-body。**
如果原文已经明显使用标题层级（例如：文章题目、`🛠 核心升级` 这种章节标题、`1. ChatGPT 端...` 这种编号小节、`事实召回率 (Factual Recall)` 这种指标小节），必须优先保留为标题层级，不要为了 label-body 把它降级成 bullet。
...
- 不要把原文中的二级 / 三级标题降级为 label-body；标题层级应在 Markdown、公众号和平台文本中都有对应体现。
...
- 是否出现重复编号：`01｜1️⃣`、`一、1.`、`01｜一、`、`1.1 1.`、`2.1 1.`。
